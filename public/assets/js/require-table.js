    ...
    var Table = {
        ...
        button: {
            ...
            reset: {
                ...
                title: '重新扫描',
                extend: 'data-toggle="tooltip"',
                classname: 'btn btn-xs btn-danger btn-reset'
            },
            dragsort: {
                name: 'dragsort',
                icon: 'fa fa-arrows',
        ...
        api: {
            ...
            bindevent: function (table) {
                ...
                // 批量重新扫描按钮事件
                $(toolbar).on('click', Table.config.resetbtn, function () {
                    var that = this;
                    var ids = Table.api.selectedids(table);
                    Layer.confirm(
                        __('Are You sure you want to reset the %s selected item?', ids.length),
                        {icon: 3, title: __('Warning'), offset: 0, shadeClose: true},
                        function (index) {
                            Table.api.multi("reset", ids, table, that);
                            Layer.close(index);
                        }
                    );
                });
            ...
            multi: function (action, ids, table, element) {
                ...
                var url = typeof data.url !== "undefined" ? data.url : (action == "del" ? options.extend.del_url : options.extend.reset_url);
            ...
            events: {
                operate: {
                    ...
                    'click .btn-reset': function (e, value, row, index) {
                        e.stopPropagation();
                        e.preventDefault();
                        var that = this;
                        var top = $(that).offset().top - $(window).scrollTop();
                        ...
                        Layer.confirm(
                            __('Are you sure you want to reset this item?'),
                            {icon: 3, title: __('Warning'), offset: [top, left], shadeClose: true},
                            function (index) {
                                var table = $(that).closest('table');
                                var options = table.bootstrapTable('getOptions');
                                Table.api.multi("reset", row[options.pk], table, that);
                                Layer.close(index);
                            }
                        );
                    }
            ...
            formatter: {
                ...
                operate: function (value, row, index) {
                    ...
                    if (options.extend.reset_url !== '' && names.indexOf('reset') === -1) {
                        buttons.push(Table.button.reset);
                    }
            ...
            buttonlink: function (column, buttons, value, row, index, type) {
                ...
                $.each(buttons, function (i, j) {
                    if (type === 'operate') {
                        ...
                        if (['add', 'edit', 'del', 'reset', 'multi', 'dragsort'].indexOf(j.name) > -1 && !options.extend[j.name + "_url"]) {
                            return true;
                        }
                    }
